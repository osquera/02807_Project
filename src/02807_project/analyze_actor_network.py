...
def load_actor_film_data(use_merged: bool = False) -> pl.DataFrame:
    ...
    if use_merged:
        ...
        if "af_film_original" not in df.columns or "af_actors_list" not in df.columns:
            raise ValueError("Merged dataset missing required actor columns")
        ...
        for row in df.iter_rows(named=True):
            ...
            if film and actors_str:
                ...
                for actor in actors:
                    if actor:
                        actor_films.append({"Film": film, "Actor": actor.strip()})
        
        return pl.DataFrame(actor_films)
    else:
        path = CLEAN_LOCATION / "actorfilms_clean.csv"
        if not path.exists():
            raise FileNotFoundError(f"Actor films dataset not found at {path}")
...
def build_movie_coactor_graph(actor_film_df: pl.DataFrame) -> nx.Graph:
    ...
    for actor, films in actor_to_films.items():
        if len(films) < 2:
            continue
...
def apply_girvan_newman(G: nx.Graph, num_communities: int = 10, sample_size: int = 50, max_nodes: int = 1000) -> dict:
    ...
    if G_connected.number_of_nodes() > max_nodes:
        logger.info(f"   Graph too large ({G_connected.number_of_nodes()} nodes), sampling {max_nodes} nodes for analysis")
        random.seed(42)
        sampled_graph_nodes = random.sample(list(G_connected.nodes()), min(max_nodes, G_connected.number_of_nodes()))
        G_to_analyze = G_connected.subgraph(sampled_graph_nodes).copy()
        logger.info(f"   Sampled subgraph: {G_to_analyze.number_of_nodes()} nodes, {G_to_analyze.number_of_edges()} edges")
    else:
        G_to_analyze = G_connected
    ...
    if G_to_analyze.number_of_nodes() > 1000:
        ...
        def most_central_edge_sampled(G):
    ...
    for i, comms in enumerate(communities_generator):
        ...
        if len(comms) >= num_communities:
            break
        if i >= 50:  # Reduced safety limit since we're sampling
    ...
    logger.info(f"✅ Girvan-Newman: Found {len(communities)} communities (+ {next_comm_id - len(communities)} isolated)")
    
    return node_to_community


def apply_louvain(G: nx.Graph) -> dict:
...
def apply_spectral_clustering(G: nx.Graph, num_clusters: int = 10) -> dict:
...
def apply_fast_spectral_clustering(G: nx.Graph, num_clusters: int = 10) -> dict:
    ...
    l = max(2, math.ceil(math.log(k, 2)))  # Number of random vectors
    t = 10 * math.ceil(math.log(n / k, 2))  # Number of power iterations
    
    logger.info(f"   Using {l} random vectors and {t} power iterations")
    
    # Get adjacency matrix and compute degree matrix
    nodes = list(G_connected.nodes())
    A = nx.to_scipy_sparse_array(G_connected, nodelist=nodes, weight='weight', format='csr')
    
    # Compute degree matrix D
    degrees = np.array(A.sum(axis=1)).flatten()
    D_inv_sqrt = csr_matrix(np.diag(1.0 / np.sqrt(degrees + 1e-10)))
    
    # Compute normalized signless Laplacian: M = D^(-1/2) (D + A) D^(-1/2)
    # This is equivalent to: I + D^(-1/2) A D^(-1/2)
    M = D_inv_sqrt @ (A + csr_matrix(np.diag(degrees))) @ D_inv_sqrt
    ...
    logger.info(f"   Running power iteration...")
    for i in range(t):
        Y = M @ Y
        if (i + 1) % 10 == 0:
    ...
    logger.info(f"   Running K-means on embedding...")
...
def calculate_modularity(G: nx.Graph, communities: dict) -> float:
...
def visualize_communities(
    ...
    for comm_id, nodes in comm_nodes.items():
        ...
        if top_node:
            ...
            label = top_node[:40] + '...' if len(top_node) > 40 else top_node
...
def analyze_communities(G: nx.Graph, communities: dict, method_name: str) -> dict:
...
def get_top_movies_per_community(
    G: nx.Graph,
    communities: dict,
    top_n: int = 5,
...
def visualize_top_movies_per_community(
    ...
    fig, axes = plt.subplots(n_communities, 1, figsize=(16, 4 * n_communities))
    ...
    for idx, (comm_id, movies) in enumerate(sorted_communities):
        ...
        # Prepare data
        movie_names = [m[0][:50] + '...' if len(m[0]) > 50 else m[0] for m in movies]  # Truncate long names
        ...
        for method, communities in top_movies_data.items():
            ...
            for comm_id, movies in communities.items():
                serializable_data[method][str(comm_id)] = [
                    {"movie": m[0], "degree": m[1], "weighted_degree": m[2]}
                    for m in movies
                ]
...
def main():
    ...
    # Build graph
    G = build_movie_coactor_graph(actor_film_df)
    ...
    louvain_top_movies = get_top_movies_per_community(G, louvain_communities, top_n=5, min_community_size=MIN_COMMUNITY_SIZE)
    top_movies_data["louvain"] = louvain_top_movies
    
    fast_spectral_top_movies = get_top_movies_per_community(G, fast_spectral_communities, top_n=5, min_community_size=MIN_COMMUNITY_SIZE)
    ...
    visualize_communities(G, fast_spectral_communities, "Fast Spectral Clustering", min_community_size=MIN_COMMUNITY_SIZE)
